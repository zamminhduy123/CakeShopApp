I. Thông tin nhóm:
-------------------------------------------------------------------------------------------------------------------------
-     MSSV     - 	     Họ và tên		-           Email cá nhân
-   18120161   - 	Nguyễn Công Danh	- 	congdanh1392@gmail.com
-   18120165   - 	Nguyễn Ngọc Đức		- 	ducnghu@gmail.com
-   18120172   - 	Nguyễn Thanh Minh Duy	- 	ntminhduy123@gmail.com
-------------------------------------------------------------------------------------------------------------------------
II. Danh sách chức năng đã hoàn thành:

1. Splash Screen (0.5 điểm):
- Hiển thị thông tin chào mừng mỗi khi ứng dụng khởi chạy.
- Mỗi lần hiện ngẫu nhiên một loại bánh ngọt nổi tiếng của một đất nước.
- Cho phép chọn check “Không hiện hộp thoại này mỗi khi khởi động”. Từ nay về sau đi thẳng vào màn hình HomeScreen.
2. Cake Screen (Home Screen) (3.5 điểm):
- Hiển thị danh sách bánh (ảnh, tên, giá nhập, giá bán, số lượng còn lại trong kho)
- Thêm/Xóa/Sửa bánh
- Lọc danh sách theo loại bánh
- Xem thông tin chi tiết bánh

3. Home Screen (Create Order Screen) (3 điểm):
- Liệt kê danh sách các món bánh mà cửa hàng bán (có thể lọc theo loại bánh và các tùy chỉnh sắp xếp)
- Thêm bánh vào đơn hàng
	+ Nhập số lượng mua trên mỗi món bánh
	+ Nhập % giảm giá trên mỗi món bánh
	+ Nhập số lượng bánh được tặng trên mỗi món bánh
- Thanh toán đơn hàng (Hiển thị Bảng hóa đơn)
	- Danh sách bánh (tên, số lượng mua, tặng, giảm giá từng món bánh)
	- Thông tin khách hàng
	- 2 Phương thức thanh toán: mua trực tiếp tại cửa hàng và đặt giao hàng
		+ Mua trực tiếp tại
			. Tiền khách đưa
			. Tiền thối lại
		+ Đặt giao hàng
			. Ngày giao hàng
			. Địa chỉ giao hàng
			. Tiền trả trước
			. Tiền trả sau
	- Giảm giá trên đơn hàng (nếu món bánh nào đã giảm giá khi thêm vào đơn hàng sẽ được chọn % giảm giá cao hơn)
	- Tổng số tiền đơn hàng

4. Statistics (3 điểm)
- Thống kê đồ thị hình cột doanh thu theo các tháng trong năm.
- Thống kê đồ thị hình bánh cho biết tỉ lệ doanh thu theo loại bánh
-------------------------------------------------------------------------------------------------------------------------
III. Các chức năng đã làm thêm:
- Tìm kiếm món bánh theo tên bánh và tìm kiếm hóa đơn theo tên khách hàng hỗ trợ cả 6 cảnh giới như đồ án 1:
	+ Cảnh giới 1: Tìm chính xác. 
	+ Cảnh giới 2: Hỗ trợ tìm kiếm không dấu. 
	+ Cảnh giới 3: Tìm không dấu hay có dấu hoặc có dấu chưa đúng nhưng kết quả vẫn ra và có độ ưu tiên.
	+ Cảnh giới 4: Tìm với từng từ hoặc kết hợp tạo ra tổ hợp từ các từ, có thể trong các trường khác nhau của dữ liệu.
	+ Cảnh giới 5: Thêm các từ khóa and, or, not.
	+ Cảnh giới 6: dùng CSDL hỗ trợ sạch các cảnh giới trên. (dùng SQL và ADO.NET framework).
- Kiểm tra các mục dữ liệu người dùng nhập vào có thỏa điều kiện cho trước hay không (Validation)

- Invoice screen: Màn hình quản lý đơn hàng
	+ Hiển thị danh sách đơn hàng (tên khách hàng, ngày thanh toán/ giao hàng, trạng thái đơn hàng, check hoàn thành đơn hàng)
	+ Xem chi tiết đơn hàng
- Cho phép bật lại splash screen
-------------------------------------------------------------------------------------------------------------------------
IV. Các chức năng chưa hoàn thành:
- Không có.
-------------------------------------------------------------------------------------------------------------------------
V. Mô hình thiết kế:
- Sản phẩm được thiết kế theo kiến trúc mô hình MVVM (Model - View - ViewModel).
-------------------------------------------------------------------------------------------------------------------------
VI. Các thư viện và package hỗ trợ:
- MaterialDesign.
- EntityFramework.
- SqlExpress.
- LiveChart.wpf.
-------------------------------------------------------------------------------------------------------------------------
VII. Các con đường bất hạnh đã xử lí:
- Thông báo nếu người dùng nhập dữ liệu lỗi(VD: ô nhập số nhưng lại nhập chữ)
- Vô hiệu hóa các nút tác động đến cơ sở dữ liệu (Thêm/Sửa) nếu người dùng muốn thêm vào bộ dữ liệu khuyết hoặc không chính xác.
- Hiện hình ảnh "No image" trong trường hợp trường dữ liệu ảnh không có ảnh.
-------------------------------------------------------------------------------------------------------------------------
VIII. Điểm đề nghị:
    10/10 điểm.
-------------------------------------------------------------------------------------------------------------------------
IX. Link video demo:
https://youtu.be/qCqEglcP31I
-------------------------------------------------------------------------------------------------------------------------
X. Link source code github:
https://github.com/zamminhduy123/CakeShopApp
-------------------------------------------------------------------------------------------------------------------------
+ Public: video Demo, file release sản phẩm
+ Private: source code
-------------------------------------------------------------------------------------------------------------------------
